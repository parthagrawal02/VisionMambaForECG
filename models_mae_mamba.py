...
class PatchEmbed(nn.Module):
    ...
    def __init__(
            self,
            img_size: Optional[int] = (12, 2500),
            patch_size: int = (1, 125),
            in_chans: int = 1,
            embed_dim: int = 768,
...
class VisionMambaEncode(nn.Module):
    def __init__(self, 
                 img_size=(12, 2500), 
                 patch_size=(1, 125), 
                 depth=24, 
    ...
    def forward_features(self, x, inference_params=None):
        ...
        if self.if_cls_token:
            return hidden_states[:, 0, :], mask, ids_restore

        if self.final_pool_type == 'none':
            return hidden_states[:, -1, :], mask, ids_restore
        elif self.final_pool_type == 'mean':
            return hidden_states.mean(dim=1), mask, ids_restore
        elif self.final_pool_type == 'max':
            return hidden_states.max(dim=1), mask, ids_restore
...
class MaskedAutoencoderVim(nn.Module):
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        for layer in self.decoder_layers:
            # rope about
            if self.if_rope:
                hidden_states = self.decoder_rope(hidden_states)
                if residual is not None and self.if_rope_residual:
                    residual = self.rope(residual)
        ...
        # return only cls token if it exists
        if self.decoder_if_cls_token:
            return hidden_states[:, 0, :]

        # # Remove cls_token
        # if self.decoder_final_pool_type == 'none':
        #     x =  hidden_states[:, -1, :]
        # elif self.decoder_final_pool_type == 'mean':
        #     x =  hidden_states.mean(dim=1)
        # elif self.decoder_final_pool_type == 'max':
        #     x = hidden_states.max(dim=1)

        elif self.decoder_final_pool_type == 'all':
            x = hidden_states
...
def mae_vim_1dcnn(**kwargs):
    model = MaskedAutoencoderVim(
        img_size=(12, 2500), patch_size=(1, 125), embed_dim=192, depth=24, 
        rms_norm=True, residual_in_fp32=True, fused_add_norm=True, 
        final_pool_type='all', if_abs_pos_embed=True, if_rope=False, 
        if_rope_residual=False, bimamba_type="v2",
